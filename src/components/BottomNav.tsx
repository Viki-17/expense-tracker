  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 safe-bottom">
      <div className="mx-auto max-w-md px-3 pb-2 pt-2">
        <div className="flex items-center justify-around rounded-[1.5rem] bg-surface/90 backdrop-blur-xl border border-separator/60 shadow-card h-[4.25rem] px-1">
          {navItems.map(({ to, label, icon: Icon, primary }) =>
            primary ? (
              <NavLink
                key={to}
                to={to}
                className="tap -mt-7 flex items-center justify-center w-14 h-14 rounded-2xl bg-accent text-on-accent shadow-lg shadow-accent/25"
                aria-label={label}
              >
